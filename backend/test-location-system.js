  ...
  try {
    ...
    if (Array.isArray(locations)) {
      ...
      if (locations.length > 0) {
        ...
        const streetViewData = await makeRequest(`http://localhost:3000/api/locations/${firstLocation.id}/streetview`);
        console.log('Street View response:');
        console.log('- Static available:', streetViewData.static?.available);
        if (streetViewData.static?.url) {
          console.log('- Static URL exists:', streetViewData.static.url.includes('maps.googleapis.com'));
        }
        console.log('- Mobile fallback used:', streetViewData.fallbackUsed || false);
      }
    } else {
