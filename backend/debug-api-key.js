...
async function debugApiKey() {
  ...
  console.log('this.apiKey length:', streetViewService.apiKey ? streetViewService.apiKey.length : 'undefined');
  console.log('!this.apiKey:', !streetViewService.apiKey);
  console.log('Boolean evaluation:', Boolean(streetViewService.apiKey));
  
  // Try the validation manually
  try {
    const result = await streetViewService.validateStreetViewCoverage(40.758896, -73.985130);
