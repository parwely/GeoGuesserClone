...
async function testStreetViewValidation() {
  ...
  const timesSquare = await streetViewService.validateStreetViewCoverage(40.758896, -73.985130);
  console.log('Times Square result:', timesSquare);

  const badLocation = await streetViewService.validateStreetViewCoverage(36.2578, 136.9061);
  ...
  const testLocations = [
    { lat: 40.758896, lng: -73.985130, name: 'Times Square' },
    { lat: 51.510067, lng: -0.133869, name: 'Piccadilly Circus' },
    { lat: 35.659515, lng: 139.700310, name: 'Shibuya Crossing' },
    { lat: 36.2578, lng: 136.9061, name: 'Rural Japan (should fail)' }
  ];

  const validationResults = await streetViewService.validateMultipleLocations(testLocations);
  
  console.log('\nValidation Summary:');
  validationResults.forEach(result => {
    const status = result.validation.hasCoverage ? '✅' : '❌';
    console.log(`${status} ${result.location.name}: ${result.validation.status}`);
