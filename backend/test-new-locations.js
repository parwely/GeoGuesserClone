...
async function testNewLocationSystem() {
  try {
    ...
    if (locations.length > 0) {
      ...
      const streetViewResponse = await fetch(`http://localhost:3000/api/locations/${timesSquare.id}/streetview`);
      ...
      if (streetViewData.static?.url) {
        console.log('- Static URL:', streetViewData.static.url.substring(0, 100) + '...');
      }
      ...
      const reliableResponse = await fetch(`http://localhost:3000/api/locations/${timesSquare.id}/streetview/reliable`);
