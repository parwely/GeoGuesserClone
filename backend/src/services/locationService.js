...
class LocationService {
  constructor() {
    ...
    this.categories = ['landmark', 'urban', 'rural', 'desert', 'mountain', 'coastal', 'forest'];
  }

  // Get random location(s) based on criteria
  async getRandomLocations(options = {}) {
    try {
      console.log('🔍 LocationService: Starting getRandomLocations with options:', options);
      ...
      // Update view count for selected locations
  ...
  async getLocationsByDifficulty(difficulty, limit = 10) {
    try {
      ...
      console.error('❌ LocationService: getLocationsByDifficulty failed:', error);
      throw error;
    }
  }
  ...
  async getLocationsByCategory(category, limit = 10) {
    try {
      ...
      console.error('❌ LocationService: getLocationsByCategory failed:', error);
      throw error;
    }
  }
  ...
  async getLocationStats() {
    try {
      ...
      const [totalResult, difficultyResult, categoryResult, countryResult, popularResult] = await Promise.all([
        // Total count
  ...
  validateLocationQuery(query) {
    ...
    if (query.difficulty && (isNaN(query.difficulty) || query.difficulty < 1 || query.difficulty > 5)) {
    ...
    if (query.count && (isNaN(query.count) || query.count < 1 || query.count > 50)) {
